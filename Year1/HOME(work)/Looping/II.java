...
public class II {
    ...
    public static void main(String[] args) {
        ...
        if (monthlyPayment <= 25) {
            balance = (purchasePrice / 12.0) * 0.0125f;
            totalpayment = monthlyPayment + balance;
        } else {
            balance = (purchasePrice / 12.0) * 0.7f;
            totalpayment = monthlyPayment + balance;
        }
        ...
        if (accountNumber <= 9999) {
            ...
            for (int month = 1; month <= 12; month++) {
                System.out.printf("Month %2d: $%.2f%n", month, totalpayment);
            }
        } else {
